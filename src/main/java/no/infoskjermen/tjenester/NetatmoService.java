...
public class NetatmoService implements PopulateInterface {
    ...
    private NetatmoMeasure getMeasures(String navn, MultiValueMap<String, String> parameters) {
        ...
        if (result == null || result.getBody() == null || result.getBody().isEmpty()) {
            log.warn("Netatmo returned empty result for parameters: " + parameters);
            return null;
        }
        ...
        try {
            ...
            log.warn("Could not log Netatmo result details: " + e.getMessage());
        }
    ...
    public NetatmoMeasure getRain(String navn) throws Exception {
        HashMap personalSettings = settings.getNetatmoSettings(navn);
        if (!personalSettings.containsKey("rain_id"))
            return null;

        log.debug("getRain");
        MultiValueMap<String, String> parameters = getRainPostParameters(navn);
        return getMeasures(navn, parameters);
    }

    public NetatmoMeasure getWind(String navn) throws Exception {
        HashMap personalSettings = settings.getNetatmoSettings(navn);
        if (!personalSettings.containsKey("wind_id"))
            return null;

        log.debug("getWind");
        MultiValueMap<String, String> parameters = getWindPostParameters(navn);
        return getMeasures(navn, parameters);
    }

    private MultiValueMap<String, String> getRainPostParameters(String navn) throws Exception {
        MultiValueMap<String, String> parameters = new LinkedMultiValueMap<>();
        HashMap personalSettings = settings.getNetatmoSettings(navn);
        parameters.set("device_id", (String) personalSettings.get("indoor_id"));
        parameters.set("module_id", (String) personalSettings.get("rain_id"));
        parameters.set("type", "Rain");
        parameters.set("date_end", "last");
        parameters.set("scale", "max");
        parameters.set("access_token", getToken(navn));
        return parameters;
    ...
    private MultiValueMap<String, String> getWindPostParameters(String navn) throws Exception {
        MultiValueMap<String, String> parameters = new LinkedMultiValueMap<>();
        HashMap personalSettings = settings.getNetatmoSettings(navn);
        parameters.set("device_id", (String) personalSettings.get("indoor_id"));
        parameters.set("module_id", (String) personalSettings.get("wind_id"));
        parameters.set("type", "WindStrength,WindAngle,GustStrength,GustAngle");
        parameters.set("date_end", "last");
        parameters.set("scale", "max");
        parameters.set("access_token", getToken(navn));
        return parameters;
    ...
    public NetatmoData getNetatmoData(String navn) throws Exception {
        return new NetatmoData(getIndoorTemperature(navn), getOutdoorTemperature(navn), getRain(navn), getWind(navn));
    }
    ...
    public boolean isPresentInSVG(String svg) {
        return svg.contains("TEMP@@") || svg.contains("LYD@@") || svg.contains("CO2@@") || svg.contains("TRYKK@@")
                || svg.contains("FUKTIGHET@@");
    }
